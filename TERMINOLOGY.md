...
| **inference artifact** | Persistent material inside an engine's lifecycle that is required or useful for inference. Examples include model weights, checkpoints, tokenizer files, compiled graphs, and installed operating-system assets. The term describes lifecycle ownership, not process location, network use, or billing. | "inspect an inference artifact", "the required inference artifacts are ready". | model artifact, downloaded model (for this concept). Use *model weights* or *checkpoint* when the specific file type is known. |
| **artifact acquisition** | An engine-owned operation that makes an inference artifact persistently available without transcribing audio. It can download, copy, extract, convert, verify, or invoke a native installer. | "explicit artifact acquisition", "artifact acquisition failed". | download (when the operation can also be local), warm-up, preparation. Use *download* only for an actual network transfer. |
| **artifact status** | A side-effect-free inspection operation and its `ArtifactReport`. It can inspect local state but does not acquire artifacts, load weights, initialize an accelerator, or run inference. | "query artifact status", "the artifact status report". | cache status, download status. |
| **artifact state** | The readiness observation for one `ArtifactRequirement`, such as `ready`, `missing`, `incomplete`, `corrupt`, or `unknown`. | "the requirement's artifact state". | status (for the per-requirement value), readiness (for the aggregate). |
| **artifact readiness** | The aggregate inference verdict in an `ArtifactReport`, derived from required requirements. | "aggregate artifact readiness", "readiness is unknown". | state (for the aggregate), status (for the aggregate value). |
| **pull** | The CLI operation that explicitly acquires inference artifacts. `pull --refresh` also re-resolves mutable source references. | "run `standard-asr pull`", "pull the required artifacts". | prepare, warm up, update. |
| **adapter** | The integration **work**: designing and writing what makes a recognizer speak the Standard ASR protocol. Its **artifact** is an engine. Name the artifact *engine* wherever it has a class, a method, an instance, or runtime state — the engine class, the engine's `_open`, engine state, the engine converts timestamps. Use *adapter* only for the work itself, the author's obligations, or a defect in the integration. The pre-standard, per-application glue the mission describes ("integrating one engine means writing an adapter") is the same word in its historical sense. | "compliance is a thin adapter, not a rewrite", "an adapter obligation", "the adapter work maps native names onto BCP-47". | Do not use for anything that has a class, an instance, or runtime state — that is the *engine*. Not a synonym for *plugin* (the package). Pydantic's `TypeAdapter` is an identifier and is exempt, as is anaphoric prose for such an object ("the adapter below"). |
...
| **provider** | The party or system on the far side of an integration. Approved in that party's compounds: `provider_params` (an engine's own escape-hatch parameters) and its prose inflections ("provider params"), "provider-native option/alias", "provider formats", and "provider storage URI" (a cloud vendor's object store). | "provider_params", "provider-native option", "provider storage URI". | Not a synonym for *engine*, *plugin*, or *distribution*. Say "distribution" for the pip package that registers an `engine_id`. |
...
| **reference server** | The HTTP and WebSocket surface that exposes any compliant engine, shipped in the `[server]` extra. It is a reference implementation, not the definition — another language may serve the same wire contract. Say **FastAPI server** only where the implementation itself is the point (a dependency, a framework detail). | "the reference server", "the FastAPI server rejects the frame". | "Web API server", "web server" (undefined extra names). |
...
| **session** | A streaming lifecycle handle: `TranscriptionSession` (async) or `SyncSession` (sync). | "streaming session", "open a session". | connection, stream (a *connection* is the transport; a *stream* is the event sequence, as in "the stream == result invariant" — neither is the session handle). |
...
| **compliance** | The conformity test surface an engine author runs: `compliance.py`, which emits a `ComplianceIssue` and a `ComplianceReport`. | "compliance suite", "compliance check". | *conformance* as a noun for the engine test surface (use "compliance"; cross-language wire conformance keeps the word); *validation* / *validate* for this step (reserve those for pydantic input validation — say "check"). The verb *conform* stays approved where an implementation conforms **to** the spec. |
...
| **frame** | Three senses, all in use. **(1) transport** — the WebSocket unit (the config frame, an audio frame, the frame-size cap). **(2) wire format** — the bare-PCM unit of the streaming wire format a session locks at `start_transcription(audio_format=...)`; the engine constrains it through `wire_encodings` and its sample-rate properties. As in "bare PCM frames" and "incremental PCM frame streaming". **(3) DSP** — one sample per channel, used only where a container or codec API names it so (a WAV header declares frames; a `pcm_s16le` frame length). | "WebSocket frame", "config frame", "bare PCM frames", "the WAV header declares N frames". | Do not use *frame* for the variable-size unit an application hands `send_audio()` or `feed()` — that is a **chunk**. |
| **strict / best_effort** | The policy axis for an unsupported input. `strict` rejects with an exception; `best_effort` accepts in a degraded form — it drops, truncates, or assumes — and emits a diagnostic. Two outcomes are policy-independent by design: a wrong-engine `provider_params` always raises, and an unsupported `candidate_languages` list never does — it resolves to `None` plus one diagnostic (spec R3; `runtime/gating.py`). | "strict mode", "best_effort policy". | — |
| **fail-\* family** | Four defined members. **fail loudly / fail-loud** — surface the problem as an exception or a diagnostic instead of proceeding silently. **fail-fast** — reject before doing expensive work. **fail-closed** — an absent, unknown, or unparseable declaration means *unsupported* or *disabled*; this is the norm here. **fail-open** — the anti-pattern in which an absent declaration reads as *supported*; name it only to describe a bug or a deliberate, documented concession. Adjectival and adverbial inflections are the same member ("a fail-loud floor", "fails loudly"). | Use a defined member. | Do not say "fail-safe" — say **fail-closed**. Do not coin any other "fail-X". |
