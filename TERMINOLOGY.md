...
| **adapter** | Informal prose word for the engine-side code an author writes, seen from the framework side. Allowed in `#` comments. In user-facing text, write "engine" instead. | Internal comments only. | In a docstring or a message, use "engine". |
