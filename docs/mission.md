...
我们的核心使命是为 ASR 推理定义一套通用的、即插即用的接口标准。如同 USB-C 统一了不同设备的物理连接一样，Standard ASR 旨在统一应用层与 ASR 模型层的交互方式。我们的目标是让开发者可以 "一次编写，运行所有" ——应用代码只需遵守 Standard ASR 标准，就能无缝地使用任何遵循该协议的 ASR 引擎，无需为每个引擎单独编写适配逻辑。

应用程序应该与具体使用的 ASR 模型完全解耦。用户应该能在应用代码完全不变的情况下，安装新的 ASR 模型。

### M.1.1: 统一流式语义是核心价值主张

这一标准不止覆盖「上传音频、等待文本」的批量调用。实时（流式）转写是当前生态碎片化最严重的领域：有的引擎发出会被反复改写的中间结果，有的引擎逐 token 吐出、一旦输出绝不更改，有的引擎在两遍重打分后合并、拆分已经发出的句段。Standard ASR 把这些行为统一为一套事件协议、段生命周期与显式的稳定性保证，让实时字幕、语音助手等流式应用代码同样做到「一次编写，运行所有」——这是本标准技术含量最高、差异化最强的部分。

### M.1.2: 「标准」分两层，保持同构

Standard ASR 的「标准」由两个保持同构的层组成：

- **进程内 Python 协议**：本地推理引擎的零拷贝层（NumPy 波形直达模型）与插件生态的宿主——引擎以独立 pip 包发布、被自动发现。ASR 引擎事实上生活在 Python 生态中，这一层是标准的根。
- **Wire 协议（HTTP / WebSocket）**：跨语言层。任何语言的应用经网络获得同构的能力——模型发现、能力查询、批量与流式转写。这一层是标准走出 Python 生态、成为「ASR 推理标准」而非「Python 生态标准」的通道。

两层共享同一能力模型（canonical JSON）、同一结果 schema、同一事件流语义；任何一层的演进必须在另一层有对应表达。
