...
(`docs/content/mission.md`); the metadata surfaces exist so apps never guess (G.1.3,
G.1.5); every new field gets a wire expression (G.5.2).

Builds on and must stay consistent with:
- `docs/content/specification/protocol.md` — §4 streaming events/stability, R5 param
