...
| `StreamClosedError` | `session.send_audio()`; `SyncSession` audio calls and iteration after the bridge tore down | Sending audio manually after `end_audio()` or after the session delivered a terminal event. The async session's `feed()` never raises it: a managed source's post-terminal chunks are discarded by design. The sync bridge is stricter: once it tore down (its `with` block exited, or a lifecycle call timed out), `feed()`, `send_audio()`, `end_audio()`, and iteration raise it -- there is no loop left to discard into. `result()` and `diagnostics()` still work. |
