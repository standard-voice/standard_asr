...
- **不变量**：present 时顶层可由 `channels` 按时间推导，两视图不冲突。**构造期强制**：`channels` 中任一通道携带 `segments`/`words` 而顶层对应字段为 `None` 的形状 MUST 在模型构造时被拒绝（不可表示）——否则"无视 channels 安全无损"的承诺被静默打破（渲染器会把仅 channels 的结果坍缩成单条无时间 cue）。
...
- `audio_format` 与 `audio` **互斥**；同时传 MUST 报错。**两者皆缺是合法的**：引擎可在适配器内部自管 wire 格式（如固定协议格式的引擎），此时 `start_transcription()` 不带任何参数即开启增量会话——标准层只在两者**同时出现**时报错。
...
| code | 含义 | recoverable |
|---|---|---|
| `done_timeout` | 超过 `done_timeout` 未收到任何事件，标准层合成终态（§6.1） | `false` |
| `stream_stalled` | 超过 `max_idle` 未收到内容事件（心跳不算），标准层合成终态 | `false` |
| `session_timeout` | 超过 `max_session_seconds` 绝对墙钟上限，标准层合成终态 | `false` |
