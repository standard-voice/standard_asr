...
- **冻结前缀保留**：`supersede` 操作 MUST 保留已冻结的文本。具体规则：被替换的旧段中，所有已冻结的文本（每段 `text[:stable_until]`），在新段的文本中 MUST 作为冻结前缀完整出现。换言之，**用户已经看到并"确信不变"的文字，在段被替换后仍然不变**。（如果旧段的冻结前缀是"你好世界"，新段的文本至少以"你好世界"开头，且其 `stable_until` ≥ 4。）
