...
- The capability and params-schema endpoints are deliberately readable without
  ...
  or authentication).
- **Validation errors never echo the request input.** Every REST **422** that
  originates from a pydantic validation failure — the global
  `RequestValidationError` handler **and** the standalone-`ValidationError` paths
  (the `options` build, engine construction, and engine-side re-validation during
  `transcribe`) — returns the **same** structured body and strips the offending
  ...
  intermediary proxy, or a copied bug report. The body is
  ...
  `loc` is anchored under the request field it came from (`["options", ...]` for
  the wire options, `["config", ...]` for engine construction). Keeping **one
  body shape per validation 422** means a cross-language client parses a single
  structure rather than discriminating string-vs-list per code.

  > A 422 that instead carries an engine-/standard-**authored** semantic message
  > (`ConfigError` / `UnsupportedFeatureError` / `InvalidProviderParamError`
  > during transcription — see §3.7) has no pydantic `loc`/`type` to expose and
  > returns the string form `{ "detail": "<authored message>" }`. These messages
  > are written for the caller (never a raw `input` echo). The two 422 forms are
  > disjoint by cause: **pydantic validation → list**, **authored semantic error
  > → string**.
...
body forms: a pydantic `ValidationError` (the `options` build, construction
validation, engine-side re-validation) returns the **structured list**
`{ "detail": [ { "type", "loc", "msg" }, ... ] }` (with the offending `input`
stripped and credential fields redacted; `loc` anchored under `["options"]` /
`["config"]`), while an authored semantic error (`ConfigError` /
`UnsupportedFeatureError` / `InvalidProviderParamError`) returns the **string**
form `{ "detail": "<authored message>" }`. The string-form messages are written
for the caller and carry no `input` echo.
...
- **In-stream error** (a `TranscriptionEvent` with `type == "error"`, produced by
  ...
  > in-process protocol (`specification.md` §4.1). Only `error` events scrub it.
