...
def validate_fetchable_url(url: str, *, allow_private_addresses: bool = False) -> None:
    ...
    # unexpected 500 in the server path) and keeps rejection symmetric across
    # IP-literal and name hosts and across both opt-in modes.
    try:
...
class ConversionPlan:
    ...
    ``operations`` is an **identifying, declarative description** of the plan
    (used for pre-call determination via :func:`can_accept` / :func:`negotiate`
    and for engine-card display), **not** an executable step pipeline.
...
def negotiate(
    ...
    # union as exhausted; the guard is for runtime, mirroring coerce_audio_input.)
    if isinstance(provided, AudioUrl):  # pyright: ignore[reportUnnecessaryIsInstance]
        return _negotiate_url(source, accepted)
    raise TypeError(
        f"negotiate() requires an AudioInput variant; got {type(provided).__name__}. "
        "Call coerce_audio_input first to wrap a bare str/bytes/ndarray."
    )
