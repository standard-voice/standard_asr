...
def _parse_entrypoint_name(name: str, *, declaration: bool = False) -> tuple[str, str, str]:
    ...
    validate_engine_id(declared_id)
...
def parse_entrypoint_name(name: str) -> tuple[str, str]:
    ...
    not necessarily the verbatim declared segment: ``my_engine`` and
    ...
    **Key Methods:**
        ...
        *,
        ...
        Args:
            ...
            shadowed_engine_ids: Engine ids contributed by more than one
                distribution (an engine-identity collision). Routing on these is
    ...
    def create(self, name: str, /, *args: Any, **kwargs: Any) -> "StandardASR":
        ...
        if engine_id in self._shadowed_engine_ids:
            # Surface the engine-identity ambiguity again at the point of use, not
            # only at discovery -- routing to a shadowed engine_id is never silent.
            logger.warning(
    ...
    *,
    ...
    for ep in found:
        ...
        if declared_id != engine_id:
            logger.info(
                "engine_id %r is not PEP 503 normalized; routing it as %r.",
                declared_id,
                engine_id,
            )

        # Count this provider for collision detection *before* ``on_conflict`` can
        # drop the spec: when two distributions expose the same model key, dropping
        # one survivor must not hide that two distributions claim this engine_id.
