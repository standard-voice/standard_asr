...
def resample_with_backend(
    audio: NDArray[np.floating], orig_sr: int, target_sample_rate: int
    ...
    """
    if orig_sr <= 0 or target_sample_rate <= 0:
    ...
    up, down = target_sample_rate // g, orig_sr // g
    out = np.asarray(
...
def resample(
    ...
    n_in = array.shape[0]
    n_out = int(round(n_in * target_sample_rate / orig_sr))
    if n_out < 1:
        n_out = 1
