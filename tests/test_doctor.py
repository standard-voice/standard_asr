...
def test_prerelease_and_epoch_pins_are_not_falsely_empty(spec: str) -> None:
    ...
    assert doctor._intersection_is_empty([SpecifierSet(spec)]) is False  # pyright: ignore[reportPrivateUsage]
...
def test_single_plugin_internally_unsatisfiable_carries_report_a_bug_hint(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """because emptiness is now an honest approximation (not a total
    soundness claim), a single-plugin 'internally unsatisfiable' verdict must
    invite a bug report rather than assert it absolutely."""
    _patch_eps(
        monkeypatch,
    ...
    report = doctor.diagnose()
    assert report.has_conflict is True
    assert any("report a bug" in c for c in report.conflicts)
