...
def test_transcribe_construction_validation_error_maps_to_422_sanitized() -> None:
    ...
    # Every REST 422 returns the same structured shape -- a list of
    # {type, loc, msg}. A construction-time error is anchored under ["config"].
    detail: list[dict[str, Any]] = resp.json()["detail"]
    assert any(entry["loc"][:1] == ["config"] and "api_key" in entry["loc"] for entry in detail)
...
def test_transcribe_engine_validation_error_maps_to_422_sanitized() -> None:
    ...
    # Structured list; the offending value travelled via the client's
