...
@pytest.mark.parametrize("bad", ["data:audio/wav;base64,AAAA", b"raw-bytes", 12345, None])
def test_negotiate_rejects_non_audioinput_with_typeerror(bad: object) -> None:
    # Negotiate is a public API; a dynamic/REST caller can pass an
    # un-coerced bare value (a str/bytes/int/None) that the static type cannot
    # stop. The old code fell through to the AudioUrl branch and silently
    # mis-negotiated such input as a URL. It must now raise a clear TypeError
    # that points at coerce_audio_input rather than producing a bogus plan.
    with pytest.raises(TypeError, match="coerce_audio_input"):
        negotiate(bad, {URL, ARR})  # type: ignore[arg-type]


@pytest.mark.parametrize(
    "url",
    [
        "https://10.0.0.1:99999/a.wav",  # private host, port out of range
        "https://internal:notaport/a.wav",  # name host, non-numeric port
    ],
)
def test_validate_url_opt_in_still_rejects_malformed_port(url: str) -> None:
    # Allow_private_addresses opts out of the private/loopback IP
    # rejection ONLY -- a malformed port is a STRUCTURAL defect that must still
    # be rejected symmetrically. The old early return skipped the port guard
    # entirely, forwarding a malformed-port URL to the engine. The opt-in path
    # must raise the same contracted UnsafeAudioUrlError as the default path.
    with pytest.raises(UnsafeAudioUrlError, match="malformed port"):
        validate_fetchable_url(url, allow_private_addresses=True)


def test_validate_url_opt_in_allows_private_host_with_valid_port() -> None:
    # With a well-formed port the opt-in still relaxes the private-IP
    # rejection as intended (the port guard does not over-reject a legal port).
    validate_fetchable_url("https://10.0.0.1:8443/a.wav", allow_private_addresses=True)
